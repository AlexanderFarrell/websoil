...
func HandleUpdateRouteWithAccount(c *fiber.Ctx, tableName string, identifierName string, allowedFields map[string]int) error {
	...
	err = UpdateRowWithAccount(account_id, tableName, identifier, identifierName, updates, allowedFields)
	if err != nil {
...
func HandleUpdateRoute(c *fiber.Ctx, tableName string, identifierName string, allowedFields map[string]int) error {
	...
	err = UpdateRow(tableName, identifier, identifierName, updates, allowedFields)
	if err != nil {
		fmt.Printf("Error updating %s: %v\n", tableName, err)
		return c.Status(500).SendString("Server error updating " + tableName)
	}
